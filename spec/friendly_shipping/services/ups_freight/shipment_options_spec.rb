...
RSpec.describe FriendlyShipping::Services::UpsFreight::ShipmentOptions do
  subject(:options) { described_class.new }

  it_behaves_like "overrideable package options class" do
    let(:default_class) { Object } # `ShipmentOptions` has multiple subclasses so we don't provide a default
    let(:required_attrs) { {} }
  end
  ...
  describe 'structure options' do
    ...
    context 'if no structure options are given' do
      ...
      it 'returns an instance of the structure options class' do
        expect(subject.options_for_structure(structure_1)).to be_a(FriendlyShipping::Services::UpsFreight::RatesStructureOptions)
      end
    end
  end
end
