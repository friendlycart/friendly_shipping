...
RSpec.describe FriendlyShipping::Services::Ups::SerializeAddressValidationRequest do
  ...
  describe '#to_xml' do
    ...
    it 'has the right data in the right places' do
      ...
      expect(subject.at_xpath('//AddressValidationRequest/AddressKeyFormat/AddressLine[2]').text).to eq('Suite 100')
      expect(subject.at_xpath('//AddressValidationRequest/AddressKeyFormat/PoliticalDivision1').text).to eq('VA')
