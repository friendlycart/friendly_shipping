...
RSpec.describe FriendlyShipping::Services::RL::ParseTransitTimesResponse do
  ...
  context "with successful response" do
    let(:response_body) { File.open(File.join(gem_root, "spec", "fixtures", "rl", "transit_timings.json")).read }
  ...
  context "with unsuccessful response" do
    let(:response_body) { File.open(File.join(gem_root, "spec", "fixtures", "rl", "zip_required.json")).read }
