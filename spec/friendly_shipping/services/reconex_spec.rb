...
RSpec.describe FriendlyShipping::Services::Reconex do
  ...
  describe "#update_load" do
    subject(:update_load) { service.update_load(shipment, options: options) }
    ...
    context "with a successful response" do
      let(:response_body) { File.read(File.join(gem_root, "spec", "fixtures", "reconex", "update_load", "success.json")) }
      ...
      it "has the correct pro_number" do
        info = update_load.value!.data
        expect(info.pro_number).to eq("123456789")
      end
    end

    context "with an error response" do
      let(:response_body) { File.read(File.join(gem_root, "spec", "fixtures", "reconex", "update_load", "failure.json")) }
      ...
      it "returns error messages" do
        expect(update_load.failure.data).to include("Load not found")
      end
    end
  end
